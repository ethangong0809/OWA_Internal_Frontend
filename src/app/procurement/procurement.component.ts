...
export class ProcurementComponent implements OnInit {
    ...
    this.http.post(url, this.requestor1)
      ...
      .subscribe((data: any) => {
        ...
        if (this.requestor1.previousStatus == 'Open' && this.requestor1.status == 'Open') {
          this.modalService.open(open1);
        }
        if (this.requestor1.previousStatus == 'Closed' && this.requestor1.status == 'Open') {
          this.modalService.open(reopen1);
        } 
        if (this.requestor1.status == 'Closed') {
          this.modalService.open(close1);
        }
  ...
  saveDocuments() {
    ...
    this.http.post(url, formData)
      ...
      .subscribe((data: any) => {
      console.log(data);
        this.modalService.open(upload1);
      this.insertView();
  ...
  deleteDocument(existingDocument) {
    ...
    this.http.post(url, existingDocument)
      ...
      .subscribe((data: any) => {
      console.log(data);
        this.modalService.open(delete1);
      this.insertView();
      }, error => {
      console.log(error);
    })
  ...
  setAssignments() {
    ...
    this.http.post(url, this.saveAssign)
      ...
      .subscribe((data: any) => {
        console.log(data);
        this.modalService.open(assign1);
        this.assignView();
        this.resetContent();
      }, error => {
        this.resetContent();
